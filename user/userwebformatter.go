...
func FormatWebUser(user UserOnWeb) UserWebFormatter {
	...
	userFormatter.Link = fmt.Sprintf("<a href='users/view/%d' class='btn btn-light'><i class='fa fa-eye'></i></a> <a href='users/edit/%d' class='btn btn-warning'><i class='fa fa-pencil'></i></a> <a href='users/delete/%d' class='btn btn-danger' onclick='javascript: return confirm(\"Are You Sure ?\")''><i class='fa fa-trash'></i></a>", user.ID, user.ID, user.ID)
